...
code from `quiz_crash_demo.py`, real traceback from
`quiz_crash_demo.crash`. Correct answer matches shown traceback --
demonstrates rendering, not recall. Compare Question 7.

```{.python include=quiz_crash_demo.py crash="true"}
```
...
```{.python include=quiz_crash_demo.py crash="true" hide_crash="true"}
```
